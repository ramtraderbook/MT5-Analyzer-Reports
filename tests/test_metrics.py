...
def test_bootstrap_risk_returns_none_below_min_trades():
    """n < MIN_TRADES_FOR_BOOTSTRAP -> None, mismo contrato que _calc_sharpe/_calc_sqn."""
    pnls = [10.0] * (MIN_TRADES_FOR_BOOTSTRAP - 1)
    assert calculate_bootstrap_risk(pnls, 10000.0) is None


def test_bootstrap_risk_returns_none_at_exactly_min_trades_minus_one_boundary():
    """El límite es estricto: N-1 -> None, N -> un dict (ver test siguiente)."""
    pnls = [10.0] * MIN_TRADES_FOR_BOOTSTRAP
    assert calculate_bootstrap_risk(pnls, 10000.0) is not None


def test_bootstrap_risk_returns_none_for_non_positive_capital():
    """capital <= 0 -> None explícito, NO el 0.0 silencioso que produce
    _calc_max_drawdown en el mismo caso (docs/known-issues.md:194-195,
    "peak_abs <= 0 y el DD% cae silenciosamente a 0.0"). Reproducir ese
    defecto conocido en código nuevo sería inexcusable."""
    pnls = [10.0] * 25
    assert calculate_bootstrap_risk(pnls, 0.0) is None
    assert calculate_bootstrap_risk(pnls, -500.0) is None


def test_bootstrap_risk_returns_none_for_empty_or_none_input():
    assert calculate_bootstrap_risk([], 10000.0) is None
    assert calculate_bootstrap_risk(None, 10000.0) is None
...
def test_bootstrap_risk_returns_none_for_nan_or_inf_in_input():
    ...
    corta con None en vez de heredar esa conducta."""
    base = [10.0] * 25
    assert calculate_bootstrap_risk(base + [float("nan")], 10000.0) is None
    assert calculate_bootstrap_risk(base + [float("inf")], 10000.0) is None
    assert calculate_bootstrap_risk(base + [float("-inf")], 10000.0) is None
...
def test_bootstrap_risk_output_shape():
    ...
    assert set(result.keys()) == {
        "max_dd_pct_p50", "max_dd_pct_p95", "max_dd_pct_p99",
